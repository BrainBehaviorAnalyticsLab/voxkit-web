  ...
  useEffect(() => {
    if (isOpen && containerRef.current && !widgetRef.current) {
      ...
      tree.addExplanation(
        ...
        [
          {
            text: "View Complete Stackers Guide",
            url: "/help/leverage-voxkit",
          },
        ],
      );
      ...
      {isOpen && (
        <div className="fixed bottom-24 right-8 w-96 h-[600px] bg-white rounded-2xl shadow-2xl border border-slate-300 z-50 overflow-hidden text-slate-900">
          <div className="relative h-full">
            ...
            <div
              ref={containerRef}
              className="h-full [&_.dt-wrapper]:text-slate-900 [&_.dt-title]:text-slate-900 [&_.dt-question-text]:text-slate-900 [&_.dt-explanation-content]:text-slate-900 [&_.dt-links-title]:text-slate-900"
            />
          </div>
        </div>
      )}
